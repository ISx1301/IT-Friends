// src/pages/api/submit-english.ts
...
function envNeed(name: string): string {
  const v = envGet(name);
  if (!v) {
    console.error(C.red(`[ENV] Missing ${name}`));
...
async function appendRowInsert(
...
function resolveBranchStrict(branchRaw: string): BranchConfig | null {
  const branch = branchRaw.toUpperCase().trim();
  const idEnv = `G_SHEETS_EN_ID_${branch}`;
  const tabEnv = `G_SHEETS_EN_TAB_${branch}`;
...
function listConfiguredBranches(): string[] {
  return envKeysWithPrefix("G_SHEETS_EN_ID_").map((k) => k.replace("G_SHEETS_EN_ID_", ""));
}
...
export const POST: APIRoute = async ({ request }) => {
  console.log(C.magenta("=== [EN Form] submit START ==="));
  ...
  try {
    ...
    if (!cfg) {
      ...
      return new Response(
        JSON.stringify({
          ok: false,
          error: `No per-branch env found for "${branch}". Expected G_SHEETS_EN_ID_${branch} (and optional G_SHEETS_EN_TAB_${branch}). Configured branches: ${configured.join(", ") || "(none)"}`,
        }),
        { status: 400 }
      );
    }
    ...
    const englishCourse   = (String(form.get("englishCourse") ?? "").trim() || null);
    ...
    const info = await transport.sendMail({
      from: mailFrom,
      to: mailTo,
      subject: `Нова заявка (Англійська, ${branch})`,
      text: textMessage,
    });
    console.log(C.cyan("[MAIL] sent OK:"), info.messageId);

    console.log(C.magenta("=== [EN Form] submit SUCCESS ==="));
    return new Response(JSON.stringify({ ok: true }), { status: 200 });
  } catch (err: any) {
    console.error(C.red("=== [EN Form] ERROR ==="), err?.message || err);
    return new Response(
